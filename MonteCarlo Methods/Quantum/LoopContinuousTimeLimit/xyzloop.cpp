...
int main(void)
{
	...
	const double beta=10,Jx=1,Jy=0,Jz=1,B=0.0;
	int i,j,k,bond[d*N][2],pos[N][d],nb[N][d][2],temp1,temp2,temp[d],s1,s2,lnum,lnum2,istep,nkink;
	double tdn,tup,tdntrial,dt,ltime,lum,lsm,Ediag1,Ediag2,Ediag,umag,smag,usus,ssus,data[8],avE,SHEAT,avUM,US,avSM,SS,smtemp,umtemp;
	...
	ofile<<"*************+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*****************"<<endl<<endl;
	ofile<<"L="<<L<<",   d="<<d<<",   beta="<<beta<<",   Jx="<<Jx<<",   Jy="<<Jy<<",   Jz="<<Jz<<endl;	
...
//			cout<<"bond   "<<"s1   "<<"s2   "<<"segment_start_time   "<<"segment_end_time   "<<"end_break_type"<<endl;
			for(i=0;i<d*N;i++)
			{
				...
				do
				{
					...
					else tdntrial=tdn-4*log(static_cast<double>(rand())/RAND_MAX)/(Jx+Jy);
//					cout<<i<<"      "<<(*p1).s<<"    "<<(*p2).s<<"          ";
					if(tdntrial>=tup) 
					{
//						cout<<tdn<<"        "<<tup<<"        ";
						...
						if(tup!=beta)
						{
							if((*p1).kkup==bond[i][1]&&(*p2).kkup==bond[i][0]&&(*p1).s!=(*p2).s) 
							{
//								cout<<"kink===>1"<<endl;
...
//	                              	cout<<"kink===>5"<<endl;
									...
									(*p1).vup=new vtx, (*p2).vup=(*p1).vup, (*p3).vdn=(*p1).vup, (*p4).vdn=(*p1).vup;
									(*(*p1).vup).p1=p1, (*(*p1).vup).p2=p2, (*(*p1).vup).p3=p3, (*(*p1).vup).p4=p4, (*(*p1).vup).type=5;
...
//	      	                cout<<tdn<<"        "<<tdntrial<<"        "<<"5"<<endl;
							...
							(*ip1).vup=(*p1).vup, (*ip2).vup=(*p2).vup;
							(*p1).vup=new vtx, (*p2).vup=(*p1).vup, (*ip1).vdn=(*p1).vup, (*ip2).vdn=(*p1).vup;
							(*(*p1).vup).p1=p1, (*(*p1).vup).p2=p2, (*(*p1).vup).p3=ip1, (*(*p1).vup).p4=ip2, (*(*p1).vup).type=5;

							vtemp=new vtxlkcell;
							(*vtemp).phead=vtx1_tail, (*vtemp).ptail=vtemp, (*vtemp).pvtx=(*p1).vup;
							(*vtx1_tail).ptail=vtemp, vtx1_tail=vtemp;

							(*p1).lupuod=1, (*p2).lupuod=1;
							(*p1).lup=p2, (*p2).lup=p1;
...
//		usus=0, ssus=0;
		for(i=1;i<lnum2;i++)
		{
			if((static_cast<double>(rand())/RAND_MAX)<=exp(-B*lp2[i].umag/2)/(exp(-B*lp2[i].umag/2)+exp(B*lp2[i].umag/2))) lp2[i].flip=-1;
			else lp2[i].flip=1;
...
/*		
		...
		for(i=1;i<lnum;i++)
		{
			cout<<i<<"               "<<lp[i].num<<"              "<<lp2[lp[i].tmp].flip<<"     "<<lp[i].len<<"     "<<lp[i].umag<<"     "<<lp[i].smag<<endl;
		}
...
*/
		...
		while(ktemp!=kk2)
		{
			Ediag2=Ediag2+umtemp*(tup-tdn);
			ssus=ssus+smtemp*(tup-tdn);
			umag=umag+abs(umtemp)*(tup-tdn);
			smag=smag+abs(smtemp)*(tup-tdn);
			umtemp=umtemp-2*((*ktemp).s1+(*ktemp).s2);
			if((*ktemp).n1%2) smtemp=smtemp+2*((*ktemp).s1-(*ktemp).s2);
			else smtemp=smtemp-2*((*ktemp).s1-(*ktemp).s2);
			ktemp=(*ktemp).ktail;
			tdn=tup, tup=(*ktemp).t;
			delete (*ktemp).khead;
		}
		...
		for(i=0;i<d*N;i++)
		{
			...
			while(tup!=beta)
			{
				Ediag1=Ediag1+(tup-tdn)*(-Jz)*(*p1).s*(*p2).s;
				tdn=tup;
